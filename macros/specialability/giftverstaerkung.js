...
const dict = {
  de: {
    noContext: "Dieses Makro benötigt einen Akteur und das auslösende Item.",
    ...
    chatSuccess: (name, oldStep, newStep) => `<b>${name}</b> wurde verstärkt: Stufe ${oldStep} → ${newStep}.`,
    ...
    typeMapEnToKey: {
      animal: "tierisch",
      "animal venom": "tierisch",
      plant: "pflanzlich",
      "plant poison": "pflanzlich",
      mineral: "mineralisch",
      "mineral poison": "mineralisch",
      alchemical: "alchimistisch",
      "alchemical poison": "alchimistisch",
    },
    removeHint: "(Rechtsklick zum Entfernen)",
    moneyItemNames: {
      ducat: ["dukaten"],
      silver: ["silber", "silberthaler", "silbertaler"],
      heller: ["heller"],
      kreuzer: ["kreuzer"],
    },
    moneyIcons: {
  ...
  en: {
    noContext: "This macro requires an actor and the source item.",
    ...
    chatSuccess: (name, oldStep, newStep) => `<b>${name}</b> has been enhanced: Step ${oldStep} → ${newStep}.`,
    stepPath: "system.step.value",
    qtyPath: "system.quantity.value",
    ...
    costCurrency: "Silver",
    typeDisplay: {
      tierisch: "animal venom",
      pflanzlich: "plant poison",
      mineralisch: "mineral poison",
      alchimistisch: "alchemical poison",
    },
    typeMatch: {
      alchimistisch: ["alchimistisch", "alchemical poison"],
      pflanzlich: ["pflanzlich", "plant poison"],
    ...
    moneyItemNames: {
      ducat: ["ducat", "ducats"],
      silver: ["silverthaler", "silverthalers"],
      heller: ["haler", "halers"],
      kreuzer: ["kreutzer", "kreutzers"],
    },
    moneyIcons: {
      ducat: "systems/dsa5/icons/money-D.webp",
      silver: "systems/dsa5/icons/money-S.webp",
...
if (!actor || !item) {
  await sendMessage(dict.noContext);
  return;
}
...
class EnhancePoisonDialog extends DialogV2 {
  ...
  _onRender(context, options) {
    ...
    this.dropZone.addEventListener("drop", async (ev) => {
      ...
      if (!typeMatches(requiredType, poisonTypeVal))
        return this.showError(dict.typeMismatch(displayType(requiredType), poisonTypeVal));
  ...
  updateInfo() {
    ...
    if (this.strengthenBtn) this.strengthenBtn.disabled = !(Number.isFinite(s) && s <= MAX_SELECTABLE_STEP);
  }
  ...
  async _onStrengthen() {
    ...
    if (!canPayObj.success) {
      const detail = canPayObj.msg ? ` ${dict.payCheckFailDetail(canPayObj.msg)}` : "";
      await sendMessage(`${dict.notEnoughMoney}${detail}`);
      return;
    }

    await payment.payMoney(actor, `${costSilver} Silbertaler`);
    await sendMessage(dict.paymentDone);
    refreshPurseInline(this.moneyInline);

    const skillName = dict.skills[requiredType];
    const skill = actor.items.find((x) => x.type === "skill" && x.name === skillName);
    if (!skill) {
      await sendMessage(`${skillName} ${lang === "de" ? "nicht gefunden." : "not found."}`);
      return;
    }
    ...
    if (qs > 0) {
      ...
      await sendMessage(dict.testFailed);
    }
  }
}
