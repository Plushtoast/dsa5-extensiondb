...
(async () => {
    ...
    const dict = {
        de: {
            ...
            itemDesc: "Es muss nicht immer Tarnele oder Wirselkraut sein. Mit der richtigen Mischung kann ein Heiler auch aus anderen, einfachen Heilkräutern eine potente Kräutermischung herstellen.",
            ...
            dialogHeader: "Die Heldin ist dazu in der Lage, eine Kräutermischung anzufertigen, die die Regeneration verbessert. Diese Mischung besteht nicht aus den üblichen Heilkräutern, die LeP regenerieren können, sondern aus einfachen, je nach Region unterschiedlichen Heilkräutern.",
            dialogFooter: "Sie benötigt dafür 4 Stunden, sofern es in der näheren Umgebung auch die nötigen Materialien gibt (z. B. in Sand- oder Eiswüsten).",
        ...
        en: {
            ...
            itemDesc: "It doesn't always have to be Tarnele or Wirsel Herb. With the right blend, a healer can make a potent herb mixture from other, simple healing herbs.",
            payLabel: "5 Silver",
            ...
            dialogHeader: "The hero can craft an herb mixture that improves regeneration. It does not consist of the usual healing herbs that regenerate HP, but of simple herbs differing by region.",
    ...
    async function processHerbMixing() {
        ...
        const setupData = await actor.setupSkill(skill, { modifier: 2, subtitle: lang === "de" ? " (Kräutermischung)" : " (Herb Mixture)" }, actor.sheet?.getTokenId?.());
    ...
    class HerbDialog extends DialogV2 {
        _onRender(context, options) {
            ...
            const enableMixing = () => {
                ...
                if (payBtn) { payBtn.style.pointerEvents = "none"; payBtn.style.opacity = "0.5"; }
                if (searchBtn) { searchBtn.style.pointerEvents = "none"; searchBtn.style.opacity = "0.5"; }
            };

            payBtn?.addEventListener("click", async (e) => {
                e.preventDefault();
                const payment = game.dsa5?.apps?.DSA5Payment;
                if (!payment) return ui.notifications.warn("DSA5Payment API nicht verfügbar.");
