...
(async () => {
    ...
    const dict = {
        de: {
            ...
            infoText: "Der Zauberer kann ein alchimistisches Elixier verdünnen, sodass daraus zwei Elixiere werden. Diese weisen jeweils eine um 2 niedrigere QS auf. Fällt die QS auf 0 oder weniger, scheitert die Verdünnung und das Elixier wird unbrauchbar.",
        ...
        en: {
            ...
            infoText: "The caster can dilute an alchemical elixir, creating two elixirs. Each has 2 lower QL. If QL drops to 0 or below, the dilution fails and the elixir becomes unusable.",
    ...
    if(validItems.length === 0) {
        listItemsHtml = `<li style="padding: 10px; color: #888; text-align: center; font-style: italic;">${dict[lang].emptyList}</li>`;
    } else {
            ...
            listItemsHtml += `
                <li class="potion-option" data-id="${item.id}" style="padding: 6px 10px; cursor: pointer; border-bottom: 1px solid rgba(0,0,0,0.1); display: flex; align-items: center; gap: 10px; transition: background 0.2s;">
                    ...
                    <div style="display: flex; flex-direction: column; line-height: 1.1;">
                        ...
                        <span style="font-size: 0.85em; color: #444;">${dict[lang].qsLabel}: ${qs}</span>
                    </div>
                </li>`;
        });
    }
    ...
    class DilutionDialog extends DialogV2 {
        constructor() {
            super({
                ...
                buttons: [
                    ...
                    {
                        action: "cancel",
                        label: dict[lang].cancel,
                        icon: "fas fa-times"
                    }
                ],
                content: `
                    <div class="dsa5" style="max-height:600px;overflow:auto;margin-bottom:10px;">
                        <p style="font-family: 'Signika'; font-weight: bold; font-size: 1.1em; border-bottom: 1px solid #736953a6;">${dict[lang].infoHeader}</p>
                        <p style="line-height: 1.5;">${dict[lang].infoText}</p>
                        <p style="font-style: italic;">${dict[lang].dropHint}<br>${dict[lang].aspCostText}</p>
            ...
            const updateDropZoneView = () => {
                if (!this.droppedItem) {
                    ...
                    dropZoneContent.innerHTML = `
                        ...
                        <div style="font-size: 0.8em; opacity: 0.6; margin-top: 5px;">(Rechtsklick zum Entfernen)</div>
                    `;
        ...
        async _onDilute() {
            ...
            if (qs > 2) {
                const newItemA = this.droppedItem.toObject();
                const newItemB = this.droppedItem.toObject();
                
                setProperty(newItemA, "system.QL", qs - 2);
                setProperty(newItemA, "system.quantity.value", 1);
                setProperty(newItemB, "system.QL", qs - 2);
